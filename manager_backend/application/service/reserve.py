...
class ReserveService():
    ...
    def get_history_record(start_date_raw, end_date_raw, num):
        ...
        if start_date_raw:
            ...
            query_condition = and_(query_condition, Reserve_Record.reserveDate>=start_date)
        if end_date_raw:
    ...
    def update_all_record_status():
        ...
        for record in records_to_update:
            ...
            if record_startTime + datetime.timedelta(minutes=valid_time)<now_datetime and record.status=="成功":
                ...
                if getStatus:
                    messageContent = ReserveService.compute_notice_message(record)
    ...
    def compute_notice_message(record):
        ...
        messageContent_data["thing8"] = {"value":"签到提醒: " + EquipmentService.get_name_by_type(record.equipmentType)[0] + str(record.equipmentID) + '号'}
