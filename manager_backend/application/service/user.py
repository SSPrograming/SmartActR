...
class UserService():
    ...
    def query_user_info(query_content):
        ...
        if query_content["stuID"] is not None:
            query_condition = and_(
                query_condition, Student.stuID == query_content["stuID"])
        if query_content["department"] is not None:
            query_condition = and_(query_condition, Student.department.like(
                '%'+query_content["department"]+'%'))
        if query_content["stuName"] is not None:
            query_condition = and_(query_condition, Student.name.like(
                '%'+query_content["stuName"]+'%'))
        ...
        if query_content["limitNum"] is None:
            ...
            if limitNum <= 0:
                return "bad arguments", False
            stuList = Student.query.filter(
                query_condition).limit(limitNum).all()
        stuInfoList = []
        for stu in stuList:
            userID = stu.userID
    ...
    def update_studentStatus(stuID, status):
        ...
        target_user = User.query.filter(
            User.userID == target_student.userID).first()
        now_datetime = now()
        now_date = datetime.date(
            now_datetime.year, now_datetime.month, now_datetime.day)
        if status == 1:
