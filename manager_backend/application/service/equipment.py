...
class EquipmentService():
    ...
    def add_equipmentType(equipmentName, Count, Description, img_name):
        ...
        获取最新生成的设备种类序号
        """
        target_equipmentType = equipmentType.query.order_by(
            equipmentType.equipmentType.desc()).first().equipmentType
        new_equipmentType.equipmentImageURL = query_yaml(
            "app.MANAGERSERVERURL") + "image/equipment/" + str(target_equipmentType) + "_" + img_name
        ...
        for i in range(Count):
            addstatus = EquipmentService.add_equipment(
                target_equipmentType, i+1)
            if not addstatus:
                return "添加设备失败", False
    ...
    def update_equipmentType(Type, new_name, new_description, new_imgname):
        ...
        if new_imgname is not None:
            new_img_url = query_yaml(
                "app.MANAGERSERVERURL") + "image/equipment/" + str(Type) + "_" + new_imgname
    ...
    def drop_related_record(Type):
        related_records = Reserve_Record.query.filter(
            Reserve_Record.equipmentType == Type).all()
    ...
    def drop_related_qrCode(Type):
        related_records = QRCode.query.filter(
            QRCode.equipmentType == Type).all()
        for record in related_records:
            try:
                db.session.delete(record)
                db.session.commit()
    ...
    def drop_related_equipment(Type):
        related_equipments = Equipment.query.filter(
            Equipment.equipmentType == Type).all()
    ...
    def get_largest_id(Type):
        ...
        if target_type is None:
            return "设备种类不存在", False
        target_equipment = Equipment.query.filter(
            Equipment.equipmentType == Type).order_by(Equipment.equipmentID.desc()).first()
        if target_equipment is None:
            return 0, True
        else:
    ...
    def get_equipment_recordList(equipmentType, equipmentID, startDate, endDate, num):
        query_condition = and_(Reserve_Record.equipmentType ==
                               equipmentType, Reserve_Record.equipmentID == equipmentID)
        if startDate is not None:
            ...
            query_condition = and_(
                query_condition, Reserve_Record.reserveDate >= startDate)
        ...
        if endDate is not None:
            try:
                ...
                return "bad arguments", False
            query_condition = and_(
                query_condition, Reserve_Record.reserveDate <= endDate)

        recordList = Reserve_Record.query.filter(query_condition).order_by(
            Reserve_Record.recordID.desc()).limit(num).all()
        return recordList, True

    def swap_equipmentOrder(Type1, Type2):
        target_type1 = equipmentType.query.filter(
            equipmentType.equipmentType == Type1).first()
        target_type2 = equipmentType.query.filter(
            equipmentType.equipmentType == Type2).first()
    ...
    def transfer_image():
        ...
        for instruction_image in instructionImage:
            instruction_image.imageURL = replace_url(
                instruction_image.imageURL)
            try:
                db.session.commit()
            except:
                db.session.rollback()
        ...
        for equipmenttype in equipmenttypelist:
            equipmenttype.equipmentImageURL = replace_url(
                equipmenttype.equipmentImageURL)
            try:
                db.session.commit()
            except:
                db.session.rollback()
        ...
        for instruction in instructionlist:
            instruction.instructionCoverURL = replace_url(
                instruction.instructionCoverURL)
            try:
                db.session.commit()
            except:
                db.session.rollback()
