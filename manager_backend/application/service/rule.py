...
class ruleService():
    def addRule(ruleType, ruleContent):
        ...
        if ruleType == 1:
            ...
            existedRules = ruleTable.query.filter(
                ruleTable.expireDate >= now_date).all()
            ...
            for record in conflict_record_list:
                try:
                    ...
                    db.session.commit()
                    if getStatus:
                        messageContent = ruleService.compute_cancel_message(
                            record)
    ...
    def get_rule_list():
        ...
        now_date = datetime.date(
            now_datetime.year, now_datetime.month, now_datetime.day)
        rule_list = ruleTable.query.filter(
            ruleTable.expireDate >= now_date).all()
        return rule_list
    ...
    def updateRule(ruleID, ruleType, ruleContent):
        ...
        now_date = datetime.date(
            now_datetime.year, now_datetime.month, now_datetime.day)
        target_rule = ruleTable.query.filter(
            ruleTable.ruleID == ruleID).first()
    ...
    def compute_cancel_message(record):
        ...
        messageContent_data["thing8"] = {"value": "预约取消提醒: " + EquipmentService.get_name_by_type(
