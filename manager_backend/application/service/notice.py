...
class NoticeService():
    def query_notice(num, query_startDate=None, query_endDate=None, query_str=None):
        ...
        if query_startDate is not None:
            query_condition = and_(
                query_condition, TableNotcie.noticeDate >= query_startDate)
        if query_endDate is not None:
            query_condition = and_(
                query_condition, TableNotcie.noticeDate <= query_endDate)
        if query_str is not None:
            query_condition = and_(
                query_condition, TableNotcie.noticeContent.like('%'+query_str+'%'))
        if num == -1:
    ...
    def update_notice(notice_content, expire_date, notice_id):
        target_notice = TableNotcie.query.filter(
            TableNotcie.noticeID == notice_id).first()
