...
def queryUserInfo():
    ...
    limitNum = request.json["limitNum"] if "limitNum" in request.json.keys(
    ) else None
    stuName = request.json["stuName"] if "stuName" in request.json.keys(
    ) else None
    department = request.json["department"] if "department" in request.json.keys(
    ) else None
...
def getEquipmentRecordList():
    ...
    recordList_raw, getStatus = UserService.get_user_recordList(
        stuID, startDate, endDate, num)
